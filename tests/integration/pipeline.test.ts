...
import { calculateCost } from '../../src/calculator/engine.js';
...
describe('Integration: Exporter mock backend', () => {
  it('exports records through the base exporter interface', async () => {
    const { CloudWatchExporter } = await import('../../src/exporters/cloudwatch.js');
  ...
  it('handles exporter errors gracefully', async () => {
    const { PhoenixExporter } = await import('../../src/exporters/phoenix.js');
